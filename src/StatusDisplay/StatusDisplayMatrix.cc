        ...
        if(colWidth[iCol] > 0){
          ...
          if(itMap != colMap.end()){
            ...
            if(itMap->second->Display(status,forceDisplay)){	      
              ...
              //Third parameter is a bitmask so that the enums and IPs are not converted to strings
              std::string cellValue = itMap->second->Print(0,true,IGNORE_ENUM | IGNORE_IP);
